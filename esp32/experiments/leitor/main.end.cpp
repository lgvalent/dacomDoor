...
String rfid_bytes_to_string(byte *array, byte n) {
  char uid[64] = {0};
  int i, k;
  byte x;
...
void read_rfid_card() {
  ...
  String rfid =
      rfid_bytes_to_string(rfid_reader.uid.uidByte, rfid_reader.uid.size);
  Serial.print("UID: ");
  Serial.println(rfid);

  reader_check_credentials(rfid, "IN");
