...
class AppTasks : public ModelControlSqliteDB
{
protected:
  time_t lastUpdate;
...
public:
  ...
  AppTasks(const char *filename) : ModelControlSqliteDB(filename) {
    this->lastUpdate = now();
  }
...
class DebugAppTasks : public AppTasks
{
public:
  DebugAppTasks(const char *filename) : AppTasks(filename) {}

  int initSession() { return SQLITE_OK; }
  void closeSession() {}
...
class DebugAppTasksDb : public AppTasks
{
public:
  DebugAppTasksDb(const char *filename) : AppTasks(filename) {}
