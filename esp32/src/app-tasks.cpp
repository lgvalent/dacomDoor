...
public:
  ...
  void value(String value) override
  {
    ...
    if (section == REMOVED && currentKey == "userId")
    {
      ...
      if (k.isValid())
      {
        ...
        Serial.print("[TASK] Removed keyring: ");
      }
      else
      {
        Serial.print("[TASK] Removed keyring not found: ");
      }
      Serial.println(uid);
    }
    ...
    if (section == UPDATED)
    {
      if (currentKey == "uid")
        uid = value;
      else if (currentKey == "userId")
        userId = value;
      else if (currentKey == "userType")
        userType = value;
      else if (currentKey == "lastUpdate")
        lastUpdate = value;
    }
  }
  ...
  void endObject() override
  {
    if (section == UPDATED && userId != "")
    {
      ...
      Serial.print("[TASK] Updated keyring: ");
...
protected:
  ...
  bool networkGuard()
  {
    if (!this->hasNetwork())
    {
      Serial.println(F("[TASK] Network was not connected"));
      return false;
    }
    return true;
  }
  ...
  bool sendUpdateKeyringsRequest()
  {
    ...
    Serial.print("[TASK] Sending keyrings update request to: ");
    Serial.println(url);
    ...
    {
      Serial.print("[TASK ERROR] StatusCode: ");
      Serial.println(code);
      http.end();
      return false;
    }
  ...
  int sendUpdateEventsRequest(std::vector<String> &result)
  {
    ...
    Serial.print("[TASK] Sending events update request to: ");
    Serial.println(url);
    ...
    if (code == 200)
      Serial.println("[TASK] All events update");
    else
    {
      Serial.printf("[TASK ERROR] Fail to update events. Code %d\n", code);
    }
    return code;
  }
  bool sendUpdateScheduleRequest()
  {
    ...
    Serial.print("[TASK] Sending schedules update request to: ");
    Serial.println(url);
    ...
    {
      Serial.println("[TASK ERROR] Fail to update schedules");
      Serial.print("[TASK ERROR] StatusCode: ");
      Serial.println(code);
    }
    else
  ...
  void updateInternalSchedules(String &strJson)
  {
    ...
    // if (error)
    // {
    //   Serial.print(F("[TASK ERROR] deserializeJson() failed: "));
    ...
    //         Serial.print("[TASK] schedule removed: ");
    //         Serial.println(id);
    //       }
    //       else
    //       {
    //         Serial.print("[TASK] schedule not found to remove: ");
    //         Serial.println(id);
    //       }
    //     }
    //     else
    //     {
    //       Serial.println("[TASK] An invalid removed entry was found!");
    //     }
    //   }
    ...
    //         Serial.print("[TASK] schedule updated: ");
    //         Serial.println(id);
    //       }
    //       else
    //       {
    ...
    //         Serial.print("[TASK] schedule added: ");
    //         Serial.println(id);
    //       }
    //     }
    //     else
    //     {
    //       Serial.println("[TASK] An invalid updated result was found!");
    //     }
    //   }
    // }
    // else
    // {
    //   Serial.println("[TASK] Request result json was different of specification!");
    // }
  }

  bool updateEvents()
  {
    Serial.println("[TASK] Updating events..");
  ...
  {
    Serial.println("[TASK] Updating schedules..");
    if (!this->networkGuard())
      return false;
  ...
  bool updateKeyrings()
  {
    Serial.println("[TASK] Updating keyrings..");
    if (!this->networkGuard())
      return false;
